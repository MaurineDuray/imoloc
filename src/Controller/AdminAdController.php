...
class AdminAdController extends AbstractController
{
    ...
    {
        //ne pas supprimer une annonce qui possède des réservations
        if(count($ad->getBookings())>0)
        {
            //pas supprimer 
            $this->addFlash(
                "warning",
                "Vous ne pouvez pas supprimer l'annonce <strong>{$ad->getId()}-{$ad->getTitle()}</strong>"
            );
        }else{
            $this->addFlash(
                "success",
                "L'annonce <strong>{$ad->getId()}-{$ad->getTitle()}</strong> a bien été supprimée"
            );
            $manager->remove($ad);
            $manager->flush();
        }
