...
class AccountController extends AbstractController
{
    ...
    /**Editer les info du user connecté */
    #[Route('/account/profile', name:"account_profile")]
    public function profile(Request $request, EntityManagerInterface $manager):Response
    {
        $user = $this->getUser(); //récup l'utilisateur connecté
        $form = $this->createForm(AccountType::class, $user); // car je ne veux pas les mots de passe
        $form->handleRequest($request);
    ...
    public function updatePassword(Request $request, EntityManagerInterface $manager, UserPasswordHasherInterface $hasher):Response
    {
        ...
        if($form->isSubmitted() && $form->isValid())
        {
            if(!password_verify($passwordUpdate->getOldPassword(),$user->getPassword())){ 
                //les erreurs viennent de visual studio code du fait que ce n'est relié à aucune entité 
